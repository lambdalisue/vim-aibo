...
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-mode)", function()
	aibo.send("\027[Z")
end, { buffer = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-verbose)", function()
	aibo.send("\015")
end, { buffer = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-todo)", function()
	aibo.send("\020")
end, { buffer = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-undo)", function()
	aibo.send("\031")
end, { buffer = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-suspend)", function()
	aibo.send("\026")
end, { buffer = true, silent = true })
vim.keymap.set({ "n", "v" }, "<Plug>(aibo-claude-paste)", function()
	aibo.send("\022")
end, { buffer = true, silent = true })
